...
const TEAMS=[
 ...
 {region:"Glazed",title:"Campeão de Tunod",mvp:"Lucario",mons:["Lucario","Gardevoir","Metagross","Blaziken","Sceptile","Dragonite"],note:"Mega Lucario Shiny • Blaziken HA • Mega Sceptile • Mega Metagross Shiny. Sceptile reaproveitado de Omega Ruby e Metagross do Emerald."},
...
const DEX_FILTERS=[["all","Todos"],["final","Time Final"],["champ","Campeões"],["mvp","MVP"],["shiny","Shiny"],["legend","Lendários"],["myth","Míticos"],["ha","Hidden Ability"],["mega","Mega Evolução"],["titles","Mais Títulos"]];
...
function passFilter(m){const b=baseKey(m.name);const f=dexFilter;
  ...
  if(f==="mvp")return(MVP_COUNT[b]||0)>0;
...
const DEX_ATTRS=[["shiny","✦ Shiny"],["ha","HA"],["mvp","MVP"],["forms","Formas Alt."],["mega","Mega"],["zmove","Z-Move"],["gmax","Gigantamax"],["legend","Lendário"],["myth","Mítico"]];
function dexAttrMatch(n,a){
  if(a==="shiny")return SHINY.has(STAGE_BASE[n]||n);
  if(a==="ha")return HA.has(n);
  if(a==="mvp")return(MVP_COUNT[baseKey(n)]||0)>0;
...
function openMon(base,region){
  ...
  const mc=MVP_COUNT[displayName]||MVP_COUNT[base]||0;
  // teams participated
  const inTeams=TEAMS.filter(t=>t.mons.some(x=>baseKey(x)===base)).map(t=>chapterName(t.region));
  const mvpIn=TEAMS.filter(t=>baseKey(t.mvp)===base).map(t=>chapterName(t.region));
  ...
  document.getElementById("modalBox").innerHTML=`
    ...
    <div class="m-body">
      ...
      <div class="m-row"><span>MVPs conquistados</span><b>${mc>=1?`⭐ ${mc}`:'Nunca foi MVP'}</b></div>
      <div class="m-row full"><span>Campeão em:</span><b>${inTeams.length?inTeams.join(", "):"—"}</b></div>
      ${mvpIn.length?`<div class="m-row full"><span>MVP em:</span><b>${mvpIn.join(", ")}</b></div>`:""}
