...
function renderMyTeams(){
  const teamMon=(name,base,sh)=>`<div class="team-mon" onclick="openMon('${base}')"><span class="mm"><img loading="lazy" src="${img(base,sh)}">${ZMOVE_CRYSTAL[base]?`<img class="z-hover" loading="lazy" src="${ZMOVE_CRYSTAL[base]}" alt="Z-Move">`:''}</span><span class="morph-name">${name}</span>${sh?'<span class="mini">✦</span>':''}</div>`;
...
function renderDex(){
  ...
  document.getElementById("dexGrid").innerHTML=list.map(m=>{
    ...
    return `<div class="card pk-card${cardClass(m.name)}${sh?' shiny-glow':''}" onclick="openMon('${b}','${m.region}')">
      ...
      <div class="pk-body"><div class="nm"><span class="morph-name">${m.name}</span>${tc>=1?`<span class="pk-title-pill">🏅${tc}</span>`:''}</div>
...
function renderPokedex(){
  ...
  document.getElementById("dexFullList").innerHTML=list.map(m=>{
    ...
    return `<div class="card pk-card${cardClass(n)}${sh?' shiny-glow':''}" onclick="openMon('${n.replace(/'/g,"\\'")}','${m.region}')">
      ...
      <div class="pk-body"><div class="nm"><span class="morph-name">${n}</span>${tc>=1?`<span class="pk-title-pill">🏅${tc}</span>`:''}</div>
